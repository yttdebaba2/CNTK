...
static shared_ptr<ComputationNode<ElemType>> CreateStandardNode(const std::wstring& nodeType, _Types&&... _Args)
{
	...
	else if (nodeType == OperationNameOf(NegSampleNode))						return New<NegSampleNode<ElemType>>(forward<_Types>(_Args)...);
...
template <class ElemType>
shared_ptr<ComputationNode<ElemType>> ComputationNetworkBuilder<ElemType>::NegSample(const ComputationNodePtr a, const ComputationNodePtr b, const std::wstring nodeName)
{
	return net.AddNodeToNetAndAttachInputs(New<NegSampleNode<ElemType>>(net.GetDeviceId(), nodeName), { a,b });
}
