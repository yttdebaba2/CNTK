...
bool MLFDeserializer::GetSequenceDescriptionByKey(const KeyType& key, SequenceDescription& result)
{
    ...
    const auto& sequence = chunk->Sequences()[chunkAndSequenceIndex.second];
    ...
    if (m_frameMode)
    {
        result.m_indexInChunk = chunk->SequenceOffsetInSamples()[chunkAndSequenceIndex.second] + key.m_sample;
