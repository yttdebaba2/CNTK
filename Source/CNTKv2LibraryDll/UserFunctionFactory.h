...
namespace CNTK
{
    ...
    public:
        void Register(const std::wstring& uniqueOpId, const std::wstring& moduleName, const std::wstring& factoryMethodName)
        {
            ...
            if (m_userFunctionFactoryMethodMap.find(uniqueOpId) != m_userFunctionFactoryMethodMap.end())
        ...
        {
            std::unique_lock<std::mutex> lock(m_mutex);
            if (m_userFunctionFactoryMethodMap.find(opId) == m_userFunctionFactoryMethodMap.end())
                InvalidArgument("UserFunction with op name '%S' has not been registered.", opId.c_str());
